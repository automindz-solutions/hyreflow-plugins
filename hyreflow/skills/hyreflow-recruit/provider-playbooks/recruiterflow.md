---
name: recruiterflow
description: "Read/write Recruiterflow (recruiting ATS/CRM) via its external API — list/get/add/update candidates, list jobs & contacts, read account. Use when the user mentions Recruiterflow, pushing candidates into the ATS, or syncing jobs/contacts. Keys: BYOK — connect your own Recruiterflow account."
---
...
## Callable surface
Call via the CLI: `hyreflow tools execute recruiterflow <method> --payload '{...}'` (preview with `--dry-run`; `hyreflow tools get recruiterflow <method>` returns the live contract + cost). Base: `https://api.recruiterflow.com/api/external`. Any endpoint without a typed method is reachable through the tool's generic `request` passthrough.
...
- `add_candidates_batch(payloads: list[dict], *, stop_on_error: bool = False) -> dict` — POST /candidate/add for many records, respecting RECRUITERFLOW_RATE_LIMIT_RPM.
- `add_client(payload: dict) -> Any` — POST /client/add — create a client company.
- `add_clients_batch(payloads: list[dict], *, stop_on_error: bool = False) -> dict` — POST /client/add for many records, rate-limited.
- `add_contact(payload: dict) -> Any` — POST /contact/add.
- `add_contact_note(payload: dict) -> Any` — POST /contact/notes/add.
- `add_contacts_batch(payloads: list[dict], *, stop_on_error: bool = False) -> dict` — POST /contact/add for many records, rate-limited.
- `add_to_campaign(payload: dict) -> Any` — POST /campaign/add-to-campaign — {campaign_id, id:[...], user_id}. Drives outreach — gate.
- `batch(method: str, calls: list, *, stop_on_error: bool = False) -> dict` — Batched get/search/update/create: run one method over many inputs, rate-limited. Each call is a value -> method(value), or {"args": [...], "kwargs": {...}} for multi-arg methods. Recruiterflow updates take a single payload dict (id inside), so update calls are plain payload dicts.
...
- `create_jobs_batch(payloads: list[dict], *, stop_on_error: bool = False) -> dict` — POST /job/create for many records, rate-limited.

...
Run via the CLI: `hyreflow tools execute clinicaltrials <method> --payload '{...}'` — method names + params are in the **Callable surface** block above; preview with `--dry-run`. (Engine base: `https://clinicaltrials.gov/api/v2`; any unwrapped endpoint is reachable through the tool's generic `request` passthrough.)

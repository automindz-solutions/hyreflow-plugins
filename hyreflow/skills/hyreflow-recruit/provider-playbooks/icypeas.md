...
- **Free operations:** `icypeas_read_results`, `icypeas_subscription_information`, and `icypeas_ping` cost zero
  Hyreflow credits. `icypeas_subscription_information` reads Icypeas' own account status and is callable
  only when the workspace has connected its own Icypeas key — budget a large run against
  `hyreflow billing balance` instead.
...
- `subscription_information(email: str | None = None) -> dict` — POST /a/actions/subscription-information — plan + remaining credits: {plan, status, credits, quotas: {daily}, currency} on the caller's own Icypeas account; requires the workspace's own Icypeas key. Requires that account's email in the body — a wrong email 401s.

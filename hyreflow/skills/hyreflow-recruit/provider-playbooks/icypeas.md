...
- **Email search is async.** `icypeas_email_search` returns a `SCHEDULED` status immediately (3 credits per
  result). Poll `icypeas_read_results` (free) with the returned `_id` to get the final email.
- **Domain search** (`icypeas_domain_search`, 0.5 credits per request) is the closest thing to a company-level
  lookup this provider offers.
- **Verify before sending.** Run `icypeas_email_verification` on discovered emails before outbound — it costs
  0.5 credits. Treat `NOT_FOUND` and `DEBITED_NOT_FOUND` as non-deliverable.

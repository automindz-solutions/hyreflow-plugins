...
## Callable surface
Call via the CLI: `hyreflow tools execute shovels <method> --payload '{...}'` (preview with `--dry-run`; `hyreflow tools get shovels <method>` returns the live contract + cost). Base: `https://api.shovels.ai/v2`. Any endpoint without a typed method is reachable through the tool's generic `request` passthrough.
...
- `search_contractors(*, geo_id: str, permit_from: str, permit_to: str, **params) -> Any` — GET /contractors/search — REQUIRES geo_id + permit_from + permit_to. Ordered by most-recent permit start. Filters: contractor_classification_derived (trade), contractor_name, permit_tags, size, cursor.

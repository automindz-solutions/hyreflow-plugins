...
Run via the CLI: `hyreflow tools execute github <method> --payload '{...}'` — method names + params are in the **Callable surface** block above; preview with `--dry-run`. (Engine base: `https://api.github.com`; any unwrapped endpoint is reachable through the tool's generic `request` passthrough.)

...
**Credit costs:** company search 0.1/result; people search 0.1/result; reverse lookup 1.4/req;
**mobile phone finder 0.4/req** (use only after a high-confidence match); export/email-finder 0.1/result;
people analysis 0.3/req; polling, statistics, results = **free**.
...
## Callable surface
Call via the CLI: `hyreflow tools execute aiark <method> --payload '{...}'` (preview with `--dry-run`; `hyreflow tools get aiark <method>` returns the live contract + cost). Base: `https://api.ai-ark.com/api/developer-portal/v1`. Any endpoint without a typed method is reachable through the tool's generic `request` passthrough.

- `company_search(payload: dict) -> dict` — POST /v1/companies — company search (CONFIRMED). Body: filters + page (0-based) + size (<=100).
- `email_finder_results(track_id: str, *, page: int | None = None, size: int | None = None) -> dict` — GET /v1/people/email-finder/{trackId}/inquiries — paginated results (page 0-based, size 1..100; CONFIRMED — path is /inquiries).
...
- `people_search(payload: dict) -> dict` — POST /v1/people — people search (CONFIRMED path). Strict nested filter structure: contact.{fullName,location,seniority,departmentAndFunction,skill,education.degree, title/duration via experience,...} and account.{domain,name,url,industries,...}, each as
- `reverse_lookup(payload: dict) -> dict` — POST /v1/people/reverse-lookup — look up a person by email/phone via `search` (CONFIRMED).
- `save_list(payload: dict) -> dict` — POST /v1/lists — create/update a list (up to 10k items) for use in the people_search `lists.people_id.exclude` filter (CONFIRMED).

---
name: aircall
description: "Read/write Aircall (cloud phone system) via its Public API — list/search/get calls, tag & comment calls, pull call transcription/summary, manage contacts, dial out, send SMS, manage webhooks. Use when the user mentions Aircall, syncing call logs/recordings into the ATS, logging call notes, or click-to-dial. Keys: not a self-serve integration yet."
---
...
## Callable surface
Call via the CLI: `hyreflow tools execute aircall <method> --payload '{...}'` (preview with `--dry-run`; `hyreflow tools get aircall <method>` returns the live contract + cost). Base: `https://api.aircall.io/v1`. Any endpoint without a typed method is reachable through the tool's generic `request` passthrough.

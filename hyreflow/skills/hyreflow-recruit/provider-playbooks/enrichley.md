---
name: enrichley
description: "Email validation via the Enrichley API — validate single emails with strong catch-all / risky-address handling to keep bounce rates low. Use when the user mentions Enrichley or validating/verifying email deliverability (esp. catch-all). Keys: hyreflow-managed credits or BYOK."
---

# Enrichley — Integration Meta Skill

HOW in `lib/enrichley.py`. Docs cached at `reference/docs/enrichley/raw/`.

## Auth & config
- **Base URL:** `https://api.enrichley.io/api/v1` · **Auth:** header `x-api-key` (env `ENRICHLEY_API_KEY`; never hardcode). Rate limit ~10 req/s, 10 concurrent.

## Operations
- `validate_email(email)` → `POST /validate-single-email`. Specialises in catch-all/risky verdicts.
- Anything else → generic `request()`.

## Handoff
Final **deliverability gate** before sending: run emails (from Apollo/Lusha/LeadMagic/FullEnrich) through Enrichley, drop undeliverable, keep catch-alls it clears. Per-row in a deterministic loop for volume.

<!-- API-SURFACE:START (auto-generated by reference/_gen_api_surface.py — do not hand-edit) -->
## Callable surface — `lib/enrichley.py`
Run via the CLI: `hyreflow tools execute enrichley <method> --payload '{...}'` — method names + params are in the **Callable surface** block above; preview with `--dry-run`. (Engine base: `https://api.enrichley.io/api/v1`; any unwrapped endpoint is reachable through the tool's generic `request` passthrough.)
- `validate_email(email: str, **opts) -> dict` — POST /validate-single-email — validate one email (handles catch-all/risky).

<!-- API-SURFACE:END -->

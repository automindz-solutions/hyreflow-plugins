...
## Callable surface
Call via the CLI: `hyreflow tools execute layoffsignal <method> --payload '{...}'` (preview with `--dry-run`; `hyreflow tools get layoffsignal <method>` returns the live contract + cost). Any endpoint without a typed method is reachable through the tool's generic `request` passthrough.

- `google_news(query: str, *, when: str | None = None, hl: str = 'en-US', gl: str = 'US', ceid: str = 'US:en', limit: int | None = None) -> list[dict]` — GET /search?q={query} — Google News RSS search (free, no key). `when` adds a recency operator (e.g. '7d','24h','1h'); hl/gl/ceid set language/country. Returns up to ~100 items as {title, link(redirector), source, published, summary}. Links are Google News redirector
- `iter_layoff_news(*, when: str = '7d', companies: list[str] | None = None, include_sites: bool = True, **kw) -> Iterator[dict]` — Walk a full sweep as one stream: the broad layoff query + one query per company (if given) + every DEFAULT_SITES feed (if include_sites). De-dup downstream by resolved URL/company.
- `layoff_news(*, when: str = '7d', company: str | None = None, terms: list[str] | None = None, **kw) -> list[dict]` — Convenience: google_news() pre-built with LAYOFF_TERMS (OR-joined). Pass `company` to scope to one employer (e.g. 'Microsoft'); `terms` to override the default layoff phrases.
- `resolve_link(url: str) -> str` — Best-effort: follow a Google News redirector link → the real publisher URL (returns the final resolved URL). Google sometimes serves a JS interstitial; if so this returns the news.google.com URL unchanged — prefer feeding such links to firecrawl, which resolves them.
- `site_feed(feed_url: str, *, limit: int | None = None) -> list[dict]` — GET {feed_url} — fetch ANY publisher RSS/Atom feed → items with CLEAN direct links (no redirector). Use for the curated high-signal outlets (see DEFAULT_SITES).
- `site_feeds(feeds: dict | None = None, *, limit_each: int | None = None) -> dict[str, list[dict]]` — Fetch several publisher feeds → {feed_name: [items]}. Defaults to DEFAULT_SITES. Per-feed errors are captured as {'error': msg} instead of aborting the sweep.

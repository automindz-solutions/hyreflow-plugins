...
## Callable surface
Call via the CLI: `hyreflow tools execute fathom <method> --payload '{...}'` (preview with `--dry-run`; `hyreflow tools get fathom <method>` returns the live contract + cost). Base: `https://api.fathom.ai/external/v1`. Any endpoint without a typed method is reachable through the tool's generic `request` passthrough.

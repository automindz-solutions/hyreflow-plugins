---
name: sourcewhale
description: "Read from and write to SourceWhale (recruiting outreach sequencer) via its public API — list campaigns/projects, add or modify candidates, add candidates into a campaign (optionally starting outreach), search candidates by email/phone/social/photo, pull dashboard statistics, manage Zapier webhooks. Use when the user mentions SourceWhale, pushing candidates into a SourceWhale campaign, syncing sourced/enriched candidates into outreach, or SourceWhale reporting."
---
...
Run via the CLI: `hyreflow tools execute sourcewhale <method> --payload '{...}'` — method names + params are in the **Callable surface** block above; preview with `--dry-run`. (Engine base: `https://sourcewhale.app/public-api`; any unwrapped endpoint is reachable through the tool's generic `request` passthrough.)
